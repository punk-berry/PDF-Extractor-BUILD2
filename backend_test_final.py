...
def test_job_status_invalid_id():
    ...
    # The server should return either 404 or 500 for invalid IDs
    assert response.status_code in [404, 500]
    print(f"✅ Invalid job ID test passed ({response.status_code} error as expected)")
    return True
...
def test_csv_download_invalid_id():
    ...
    # The server should return either 404 or 500 for invalid IDs
    assert response.status_code in [404, 500]
    print(f"✅ Invalid job ID CSV download test passed ({response.status_code} error as expected)")
    return True
